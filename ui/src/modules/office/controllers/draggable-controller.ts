...
export class DraggableController {
  ...
  constructor(
    ...
    private constrainPosition?: DragPositionConstraint,
  ) {}
  ...
  private updateDrag(mouseEvent: MouseEvent): void {
    ...
    const newPosition = new THREE.Vector3().addVectors(this.planeIntersect, this.dragOffset);

    // Snap to grid
    const snappedPosition = this.constrainPosition
  ...
  private snapToGrid(position: THREE.Vector3): THREE.Vector3 {
    const { cellSize, worldOffsetX, worldOffsetZ, gridWidth, gridDepth } = getGridData();
    ...
    const gridX = Math.max(
      0,
      Math.min(gridWidth - 1, Math.round((position.x + worldOffsetX) / cellSize)),
    );
    const gridZ = Math.max(
      0,
      Math.min(gridDepth - 1, Math.round((position.z + worldOffsetZ) / cellSize)),
    );
