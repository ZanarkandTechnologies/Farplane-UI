...
import { buildOfficeObjectMetadata, parseOfficeObjectInteractionConfig } from "../office-object-ui";
...
export function usePlacementSystem() {
  ...
  async function placeGenericObject(input: {
    ...
    const meshType = builtInMeshTypes.includes(input.type as (typeof builtInMeshTypes)[number])
  ...
  const startPlacement = useCallback(
    (type: PlacementType, data: Record<string, unknown>) => {
  ...
  const confirmCoordinatePlacement = useCallback(
  ...
  const confirmTeamAssignment = useCallback(
    async (teamId: string) => {
      if (!type) return;
