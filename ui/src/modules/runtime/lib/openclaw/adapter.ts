...
export class OpenClawAdapter {
  ...
  async installRepoSkillToAgentWorkspace(
    ...
    return this.postWorkspaceSkillMutation("/openclaw/skills/install-workspace", {
      agentId,
      skillId,
    });
  }
  ...
  async removeAgentWorkspaceSkill(
    ...
    return this.postWorkspaceSkillMutation("/openclaw/skills/remove-workspace", {
      agentId,
      skillId,
    });
  }
  ...
  async generateMeshyAsset(input: {
    ...
    try {
      ...
      if (payload.ok === false) {
        return {
          ok: false,
          error: typeof payload.error === "string" ? payload.error : "mesh_generation_failed",
        };
      }
      const asset = toMeshAsset(payload.asset);
      if (!asset) return { ok: false, error: "mesh_asset_invalid" };
