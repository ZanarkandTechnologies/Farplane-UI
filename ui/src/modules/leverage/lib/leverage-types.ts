/** Browser-safe read model for the global, read-only Leverage panel. */
...
export type LeverageDistribution = {
  metricId: string;
  label: string;
  observedAt: string | null;
  status: string;
...
export type LeverageProjectionResponse = {
  ok: boolean;
  projection?: LeverageProjection;
  error?: string;
};
