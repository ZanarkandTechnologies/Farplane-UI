...
function formatThinkingSummary(part: Extract<LocalChatPart, { kind: "thinking" }>): string {
  ...
  if (part.encrypted)
    return "Reasoning signature stored, but transcript does not contain readable thinking text.";
...
function createAssistantMessage(
  text: string,
  kind: LocalChatMessage["kind"] = "default",
): LocalChatMessage {
  return {
...
function isWorkingOutputMessage(message: LocalChatMessage): boolean {
  ...
  return (
    text.startsWith("[tool:") || text.startsWith("[lifecycle:") || text.startsWith("[assistant]")
  );
}
...
function mapTimelineEventToLocal(
  event: { role: string; text: string; ts: number; type?: string; eventId?: string },
  index: number,
): LocalChatMessage[] {
...
export function useChatMessages(
  ...
  const activeThread = useMemo(
    () => threads.find((thread) => thread._id === effectiveThreadId),
    [effectiveThreadId, threads],
  );
  const effectiveSessionKey =
    activeThread?.sessionKey ??
    (activeThread?.conversationKey ? "codex-main" : (selectedSessionKey ?? threadId));
    ...
    async function loadHistory(): Promise<void> {
      try {
        ...
        useChatStore.getState().setMessagesByThread({
          ...
          [effectiveThreadId ?? sessionKey]: mapped,
        });
      } catch {
        if (!cancelled) {
  ...
  }, [effectiveSessionKey, effectiveThreadId, reloadHistory]);
    ...
    const timer = window.setTimeout(() => {
      void reloadHistory(effectiveSessionKey)
        .then((mapped) => {
          ...
          if (!lastMessage || lastMessage.role !== "assistant" || mapped.length <= startedCount)
            return;
          useChatStore.getState().setMessagesByThread({
            ...useChatStore.getState().messagesByThread,
            [effectiveThreadId]: mapped,
  ...
  const handleSubmit = useCallback(
    async (messageParam: unknown): Promise<void> => {
      if (!selectedAgentId || !effectiveSessionKey || !effectiveThreadId) {
        const nextMessages =
          useChatStore.getState().messagesByThread[effectiveThreadId ?? "unbound"] ?? [];
        if (effectiveThreadId) {
          useChatStore.getState().setMessagesByThread({
            ...useChatStore.getState().messagesByThread,
            [effectiveThreadId]: [
              ...nextMessages,
              createAssistantMessage("Error: no active agent/session selected"),
            ],
          });
        }
        setSubmissionStatus("ready");
      ...
      try {
        if (adapter.runtimeKind === "codex") {
          ...
          if (activeThread?.conversationKey) {
            setThreads(
              useChatStore
                .getState()
                .threads.map((thread) =>
                  thread._id === activeThread._id
                    ? { ...thread, sessionKey: resolvedSessionKey, isPendingNew: false }
                    : thread,
                ),
            );
          } else if (resolvedSessionKey !== selectedSessionKey) {
            setSelectedSessionKey(resolvedSessionKey);
          }
          const mapped = await reloadHistory(resolvedSessionKey);
          useChatStore.getState().setMessagesByThread({
            ...useChatStore.getState().messagesByThread,
            [activeThread?.conversationKey ? activeThread._id : resolvedSessionKey]:
              mapped.length > 0
                ? mapped
        ...
        useChatStore.getState().setMessagesByThread({
          ...useChatStore.getState().messagesByThread,
          [effectiveThreadId]: [...nextMessages, createAssistantMessage(`Error: ${visibleError}`)],
        });
        setStreamingText("");
        setActiveRunId(null);
        setSubmissionStatus("ready");
