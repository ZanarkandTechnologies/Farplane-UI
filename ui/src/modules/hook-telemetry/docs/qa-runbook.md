...
3. Open the office launcher and choose `Raw Telemetry`.
4. Verify `Events` renders either a table of hook rows or the empty state.
5. Switch to `Distribution` and verify event/hook/session bars fit in the panel.
6. Switch to `Hooks` and verify the install command, `/hooks` trust chip, and
   file matcher preview render without nested scrolling fights.
7. Disable Convex for the UI session and verify the unavailable state appears.
8. Open the office in viewer/public mode and verify `Raw Telemetry` is not
