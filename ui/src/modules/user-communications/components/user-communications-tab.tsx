...
type UserCommunicationsTabProps = {
  /** Renders only the typed settings form for Settings → Configs. */
  settingsOnly?: boolean;
};

export function UserCommunicationsTab({
  settingsOnly = false,
}: UserCommunicationsTabProps): ReactElement {
  ...
  const [settingsOpen, setSettingsOpen] = useState(settingsOnly);
    ...
    <div className="rounded-md border bg-muted/20 p-3">
      ...
      <div className="mt-3 grid gap-2 border-t pt-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
        ...
        <Tooltip>
          ...
          <TooltipContent>
            Copies the environment-injected gateway command. Non-secret settings save to
  ...
  return (
    <div className={settingsOnly ? "space-y-3" : "flex h-full min-h-0 flex-col overflow-hidden"}>
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {settingsOnly ? null : <Badge variant="outline">state: local</Badge>}
              {!settingsOnly && stateError ? (
                <Badge variant="outline">state unavailable</Badge>
              ) : null}
            </div>
            {settingsOnly ? null : (
      ...
      {settingsOnly ? (
        ...
        <Collapsible open={settingsOpen} onOpenChange={setSettingsOpen}>
          <CollapsibleContent>
            <div className="my-3">{gatewaySettings}</div>
          </CollapsibleContent>
        </Collapsible>
      )}
