 ...
 * CAPITAL DETAILS
 * ===============
 * Finance-owned read-only detail rendered inside the Leverage workspace.
 *
 * Input: browser-safe Finance projection plus query state.
 * Output: capital balance, flow, history, and source-gap sections.
 * Invariant: Finance owns cash facts; Leverage owns the single operator panel.
 */
...
import type { FinancePeriodTotals, FinanceProjection } from "./lib/finance-types";

type FinanceCapitalDetailsProps = {
  error: string | null;
  isLoading: boolean;
...
export function FinanceCapitalDetails({
  error,
  isLoading,
  projection,
}: FinanceCapitalDetailsProps): ReactElement {
  if (isLoading) {
    return (
      <div
        role="status"
  ...
  if (error) {
    return (
      ...
      >
        ...
        Capital data is unavailable. Refresh to try again.
      </div>
    );
  }
  ...
  if (!projection) {
    return (
      <div className="grid h-32 place-items-center border border-border/80 text-sm text-muted-foreground">
        No capital evidence is available yet.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5" data-testid="leverage-capital-details">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
        <span>{syncLabel}</span>
        <span aria-hidden="true">·</span>
        <span>{projection.observationCount} source observations</span>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-[1.35fr_1fr]">
        ...
        <Card className="rounded-none border-border/80 py-0">
          ...
          <CardContent className="px-5 pb-5">
            ...
            <FlowBreakdown totals={projection.currentMonth} currency={projection.currency} />
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.25fr_1fr]">
        <section className="border border-border/80 bg-card">
          <div className="border-b border-border/80 px-4 py-3">
            <h4 className="text-sm font-semibold">Cash Balance History</h4>
          </div>
          <div className="divide-y divide-border/70">
            {balanceHistory.length ? (
              balanceHistory.slice(0, 12).map((row) => (
                <div
                  key={`${row.asOf}:${row.observedAt}`}
                ...
                >
                  ...
                  <span
                    className={`ml-auto font-mono font-semibold tabular-nums ${tone(row.balanceCents)}`}
                  >
        ...
        <section className="border border-border/80 bg-card">
          <div className="border-b border-border/80 px-4 py-3">
            <h4 className="text-sm font-semibold">Weekly Closes</h4>
          </div>
          <div className="divide-y divide-border/70">
            {projection.weeklyHistory.length ? (
              projection.weeklyHistory.slice(0, 8).map((row) => (
                <div key={row.weekKey} className="flex items-center gap-3 px-4 py-3 text-sm">
                  <div>
      ...
      <section className="border border-border/80 bg-card">
        <div className="border-b border-border/80 px-4 py-3">
          <h4 className="text-sm font-semibold">Daily Flow</h4>
        </div>
            ...
            projection.daily.slice(0, 14).map((row) => (
              <div key={row.date} className="space-y-2 px-4 py-3 text-xs">
                <div className="flex items-center justify-between gap-3">
                  <span className="font-mono text-muted-foreground">{displayDate(row.date)}</span>
                  <span
                ...
                <div className="flex justify-between gap-4 text-muted-foreground">
                  <span>Income {formatter(projection.currency).format(row.incomeCents / 100)}</span>
                  <span>
                    Expenses {formatter(projection.currency).format(row.expenseCents / 100)}
                  </span>
                </div>
              </div>
            ))
              ...
              {projection.daily.length ? (
                projection.daily.slice(0, 14).map((row) => (
                  <TableRow key={row.date}>
                    <TableCell className="font-mono text-xs">{displayDate(row.date)}</TableCell>
      ...
      {projection.sourceGaps.length ? (
        <div
          role="status"
          data-testid="leverage-capital-source-gaps"
