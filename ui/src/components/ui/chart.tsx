...
function ChartStyle({ id, config }: { id: string; config: ChartConfig }): React.ReactElement | null {
  ...
  return (
    <style
      // biome-ignore lint/security/noDangerouslySetInnerHtml: Chart theme variables are generated from static config keys and colors.
...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [item] = payload;
    const key = getStringKey(labelKey ?? item?.dataKey ?? item?.name, "value");
  ...
  return (
    <div
      className={cn(
        "grid min-w-32 items-start gap-1.5 rounded-md border border-border/70 bg-background px-2.5 py-1.5 text-xs shadow-xl",
        className,
      )}
    >
      ...
      <div className="grid gap-1.5">
        {payload
          ...
          .map((item, index) => {
            const key = getStringKey(nameKey ?? item.name ?? item.dataKey, "value");
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
...
function getStringKey(value: unknown, fallback: string): string {
