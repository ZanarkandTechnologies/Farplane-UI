 ...
 * CEO WORKBENCH PANEL
 * ===================
 * Read-only CEO workspace projected from canonical filesystem tickets.
 *
 * KEY CONCEPTS:
...
export function CeoWorkbenchPanel(): JSX.Element {
  ...
  const { companyModel, isLoading } = useOfficeDataContext();
  const [selectedTaskKey, setSelectedTaskKey] = useState<string | null>(null);

  const tasks = useMemo(() => resolveReviewBoardTasks(companyModel?.tasks), [companyModel?.tasks]);

  const selectedTask = useMemo(
    () => tasks.find((task) => reviewBoardTaskKey(task) === selectedTaskKey) ?? null,
      ...
      <DialogContent className="z-[1000] flex h-[min(92vh,920px)] max-w-[96vw] flex-col overflow-hidden border border-border bg-background p-0 text-foreground shadow-2xl sm:max-w-[1520px]">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="space-y-3">
              ...
              <DialogDescription className="max-w-4xl text-sm leading-7 text-muted-foreground">
                Filesystem ticket workspace for CEO tasks and review queues. Open any card to read
                its canonical task memory; lifecycle changes stay in the ticket workflow.
              </DialogDescription>
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className={`h-10 border px-4 text-sm uppercase tracking-wide transition ${
                  view === "board"
                    ? "border-primary bg-primary/15 font-semibold text-primary shadow-[inset_0_-2px_0_0_hsl(var(--primary))]"
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.18em] text-muted-foreground">
              ...
              {view === "review" ? (
                <span className="inline-flex items-center gap-2 text-muted-foreground">
                  ...
                  Reviewing {reviewTasks.length} tasks across filesystem tickets
                </span>
              ) : null}
            </div>
          </div>
        ...
        <ScrollArea className="min-h-0 flex-1">
          {isLoading ? (
            ...
            <div className="m-6 border border-destructive/40 bg-destructive/10 p-5 text-sm text-destructive">
              The company ticket source is unavailable, so tasks cannot be shown.
            </div>
          ) : tasks.length === 0 ? (
            <div className="m-6 border border-dashed border-border bg-card p-5 text-sm text-muted-foreground">
              No active filesystem tickets were found across tracked projects.
            </div>
          ) : (
            <div
                ...
                <section key={lane.key} className={`border bg-card ${lanePalette(lane.key)} p-4`}>
                  ...
                  <div className="space-y-3">
                    {lane.tasks.length === 0 ? (
                      ...
                      lane.tasks.map((task) => (
                        <button
                          key={reviewBoardTaskKey(task)}
                          type="button"
                          onClick={() => setSelectedTaskKey(reviewBoardTaskKey(task))}
                        ...
                        >
                          ...
                          <div className="mt-4">
                            <TaskMemoryView
                              notes={
                                task.markdown?.trim()
                                  ? stripYamlFrontMatter(task.markdown)
                                  : task.notes
                              }
                              variant="compact"
                            />
