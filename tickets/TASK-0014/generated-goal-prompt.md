...
# TASK-0014 Generated Goal Prompt

```text
/goal Run the following files as one Goal Packet.
Files:
- tickets/TASK-0014/ticket.md
- tickets/TASK-0014/program.md
- tickets/TASK-0014/progress.md
- tickets/TASK-0014/generated-goal-prompt.md
...
Task: Complete TASK-0014 end to end. Make the operator's three-phase algorithm
the canonical automatic office layout pipeline:
1. decide/render the strategy graph or area seed, compact it, and reserve
   shortest walking paths;
2. optimize optional object placement around those reserved paths;
3. remove empty edge tiles to lower empty score without breaking required
   access, blocked paths, or important POI reachability.
Preserve manual layout as the only bypass and do not restore generated walls.
Treat the listed ticket and program files as source of truth.

Logging: Before ending each turn, append a compact progress entry to
tickets/TASK-0014/progress.md when ticket state, implementation state,
...
office evidence for Team Neighborhoods plus at least one other automatic
strategy when the local UI launches cleanly.
...
Approval: approved. The operator explicitly requested Goal Advisor to create
and run this goal until the office auto-layout becomes the operator's
